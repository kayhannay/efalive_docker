...
RUN echo "deb http://cdn-fastly.deb.debian.org/debian buster main" > /etc/apt/sources.list && echo "deb http://cdn-fastly.deb.debian.org/debian buster-updates main" >> /etc/apt/sources.list && echo "deb http://security.debian.org buster/updates main" >> /etc/apt/sources.list
